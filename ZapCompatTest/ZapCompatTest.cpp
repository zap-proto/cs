...
using namespace zap;
using namespace kj;
using namespace zap_test::zap::test;
...
void BasicTest(zap::EzRpcClient& rpc)
{
...
void PipeliningTest(zap::EzRpcClient& rpc)
{
...
void ReleaseTest(zap::EzRpcClient& rpc)
{
...
void ReleaseOnCancelTest(zap::EzRpcClient& rpc)
{
...
void TailCallTest(zap::EzRpcClient& rpc)
{
...
void CancelationTest(zap::EzRpcClient& rpc)
{
...
void PromiseResolveTest(zap::EzRpcClient& rpc)
{
...
void RetainAndReleaseTest(zap::EzRpcClient& rpc)
{
...
void CancelTest(zap::EzRpcClient& rpc)
{
...
void SendTwiceTest(zap::EzRpcClient& rpc)
{
...
void EmbargoTest(zap::EzRpcClient& rpc)
{
...
void EmbargoErrorTest(zap::EzRpcClient& rpc)
{
...
void EmbargoNullTest(zap::EzRpcClient& rpc)
{
...
void CallBrokenPromiseTest(zap::EzRpcClient& rpc)
{
	...
	auto req = client.callHeldRequest().send()
		.then([&](zap::Response<TestMoreStuff::CallHeldResults>&&) {
		returned = true;
	}, [&](kj::Exception&& e) {
		returned = true;
...
int main(int argc, const char* argv[])
{
	...
	if (strncmp(argv[1], "client", 6) == 0)
	{
		try
		{
			zap::EzRpcClient client(argv[2], 33444);
	...
	else if (strncmp(argv[1], "server", 6) == 0)
	{
		...
		zap::EzRpcServer server(mainInterface, argv[2], 0);
