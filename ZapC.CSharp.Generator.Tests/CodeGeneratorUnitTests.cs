using Zap;
using Model = ZapC.CSharp.Generator.Model;
using CodeGen = ZapC.CSharp.Generator.CodeGen;
using CodeGeneratorRequest = ZapC.CSharp.Generator.Schema.CodeGeneratorRequest;
...
namespace ZapC.CSharp.Generator.Tests
{
    ...
    public class CodeGeneratorUnitTests
    {
        ...
        {
            var model = Load("UnitTest1.zap.bin");
        ...
        {
            var (model, codegen, _) = LoadAndGenerate("UnitTest1.zap.bin");
        ...
        {
            LoadAndGenerate("UnitTest2.zap.bin");
        }

        [TestMethod]
        public void Test03NonGeneratedNodeSkip()
        {
            LoadAndGenerate("UnitTest3.zap.bin");
        }

        [TestMethod]
        public void Test04MutualDependencies()
        {
            LoadAndGenerate("UnitTest4.zap.bin", "UnitTest4b.zap.bin");
        }

        [TestMethod]
        public void Test10ImportedNamespaces()
        {
            var (model, codegen, _) = LoadAndGenerate("UnitTest10.zap.bin", "UnitTest10b.zap.bin");
            var outerTypeDef = GetGeneratedFile("UnitTest10.zap", model).NestedTypes.First();
        ...
        {
            LoadAndGenerate("UnitTest11.zap.bin", "UnitTest11b.zap.bin");
        }

        [TestMethod]
        public void Test12ConstEnum()
        {
            LoadAndGenerate("UnitTest12.zap.bin");
        }

        [TestMethod]
        public void Test13CSharpNamespace()
        {
            var (model, _, _) = LoadAndGenerate("UnitTest13.zap.bin");
            var outerTypeDef = GetGeneratedFile("UnitTest13.zap", model).NestedTypes.First();
            string[] outerNamespace = { "Foo", "Bar", "Baz" };
            CollectionAssert.AreEqual(outerNamespace, (outerTypeDef.DeclaringElement as Model.GenFile).Namespace);
        }
        ...
        {
            var (model, _, _) = LoadAndGenerate("UnitTest14.zap.bin");
            var outerTypeDef = GetGeneratedFile("UnitTest14.zap", model).NestedTypes.First();
            string[] outerNamespace = { "Foo", "Bar", "Baz" };
            CollectionAssert.AreEqual(outerNamespace, (outerTypeDef.DeclaringElement as Model.GenFile).Namespace);
        }
        ...
        {
            var (_, _, code) = LoadAndGenerate("UnitTest15.zap.bin");
            try
            {
        ...
        {
            LoadAndGenerate("UnitTest20.zap.bin");
        }

        [TestMethod]
        public void Test30SchemaZap()
        {
            LoadAndGenerate("schema-with-offsets.zap.bin");
        }

        [TestMethod]
        public void Issue45()
        {
            var input = CodeGeneratorSteps.LoadResource("Issue45.zap.bin");
            using (input)
            {
                var frame = Framing.ReadSegments(input);
                var ds = DeserializerState.CreateRoot(frame);
                var cgr = ZapSerializable.Create<CodeGeneratorRequest>(ds);
        ...
        static (Model.SchemaModel, CodeGen.CodeGenerator, string) LoadAndGenerate(params string[] inputNames)
        {
            ...
            Assert.AreEqual(
                ...
                Util.InlineAssemblyCompiler.TryCompileZap(
