...
namespace ZapC.CSharp.Generator.Tests
{
    ...
    public class CodeGeneratorSteps
    {
        ...
        internal static bool IsZapInstalled()
        {
            try
            {
                var startInfo = new ProcessStartInfo(ZapCompilation.ZapCompilerFilename, "--version");
                ...
                {
                    Assert.IsNotNull(process, $"Unable to start '{ZapCompilation.ZapCompilerFilename}'");
        ...
        public void WhenIInvokeZapc_Csharp()
        {
            Console.WriteLine($"Generate nullable reference types? {_nullableGenEnable}");

            using (_inputStream)
            {
                _result = ZapCompilation.GenerateFromStream(_inputStream, new CodeGen.GeneratorOptions()
                {
        ...
        public void ThenTheGeneratedOutputMustMatchTheReference()
        {
            ...
            if (!equals)
            {
                string path = Path.ChangeExtension(Path.GetTempFileName(), ".zap.cs");
        ...
        public void GivenZap_ExeIsInstalledOnMySystem()
        {
            if (!IsZapInstalled())
            {
                Assert.Inconclusive("Zap compiler not found. Precondition of this test is not met.");
            }
        }

        [Given(@"I have a schema ""(.*)""")]
        public void GivenIHaveASchema(string zapFileName)
        {
            _inputSchemaFileName = zapFileName;

            using (var stream = LoadResource(zapFileName))
            using (var reader = new StreamReader(stream))
            {
        ...
        public void WhenIWantToGenerateCodeFromThatSchema()
        {
            ...
            _result = ZapCompilation.InvokeZapAndGenerate(new string[] { path });
        }
        ...
        public void GivenZap_ExeIsNotInstalledOnMySystem()
        {
            if (IsZapInstalled())
            {
                Assert.Inconclusive("Zap compiler found. Precondition of this test is not met.");
            }
        }
        ...
        {
            Assert.IsTrue(_result.ErrorCategory == ZapProcessFailure.BadInput);
        }
        ...
        public void ThenTheInvocationMustSucceedAndAttemptingToCompileTheGeneratedCodeGives(string result)
        {
            ...
            var summary = Util.InlineAssemblyCompiler.TryCompileZap(
