    ...
    class InterfaceSnippetGen
    {
        ...
        StatementSyntax MakeProxyCreateResult(Method method)
        {
            ...
            ExpressionSyntax createDomain = InvocationExpression(
                    MemberAccessExpression(
                        SyntaxKind.SimpleMemberAccessExpression,
                        IdentifierName(nameof(Zap.ZapSerializable)),
                        GenericName(nameof(Zap.ZapSerializable.Create))
        ...
        public MemberDeclarationSyntax MakeProxy(TypeDefinition type)
        {
            var classDecl = ClassDeclaration(_names.MakeTypeName(type, NameUsage.Proxy).Identifier)
                ...
                .AddModifiers(Public)
                .AddBaseListTypes(
                    SimpleBaseType(_names.Type<Zap.Rpc.Proxy>(Nullability.NonNullable)),
            ...
            foreach (var method in allMethods)
            {
                                    ...
                                    .WithInitializer(
                                        EqualsValueClause(
                                            InvocationExpression(
                                                MemberAccessExpression(
                                                    SyntaxKind.SimpleMemberAccessExpression,
                                                    IdentifierName(nameof(Zap.SerializerState)),
                                                    GenericName(
                                                        Identifier(nameof(Zap.SerializerState.CreateForRpc)))
                ...
                var call = InvocationExpression(IdentifierName(nameof(Zap.Rpc.BareProxy.Call)))
                    .AddArgumentListArguments(
                        Argument(
                        ...
                        Argument(
                            InvocationExpression(
                                MemberAccessExpression(
                                    ...
                                    GenericName(nameof(Zap.SerializerState.Rewrap))
                                        .AddTypeArgumentListArguments(_names.Type<Zap.DynamicSerializerState>(Nullability.NonNullable))))
                                        .AddArgumentListArguments()),
                        Argument(
                ...
                if (IsSubjectToPipelining(method))
                {
                    ...
                    var pipelineAwareCall = InvocationExpression(
                        MemberAccessExpression(
                            SyntaxKind.SimpleMemberAccessExpression,
                            IdentifierName(nameof(Zap.Rpc.Impatient)),
                            IdentifierName(nameof(Zap.Rpc.Impatient.MakePipelineAware))))
                            .AddArgumentListArguments(
                                Argument(call),
                ...
                else
                {
                    ...
                    var whenReturned = MemberAccessExpression(
                        SyntaxKind.SimpleMemberAccessExpression,
                        call,
                        IdentifierName(nameof(Zap.Rpc.IPromisedAnswer.WhenReturned)));
                    ...
                    yield return Argument(
                        GenericName(_names.GetCodeIdentifier(method).ToString())
                            .AddTypeArgumentListArguments(
                                Enumerable.Repeat(
                                    _names.Type<Zap.AnyPointer>(Nullability.NonNullable),
            ...
            else
            {
                ...
                var maybeTailCall = InvocationExpression(
                    MemberAccessExpression(
                        SyntaxKind.SimpleMemberAccessExpression,
                        IdentifierName(nameof(Zap.Rpc.Impatient)),
                        IdentifierName(nameof(Zap.Rpc.Impatient.MaybeTailCall))))
                        .AddArgumentListArguments(
                            Argument(call),
                            Argument(lambdaArg));
        ...
        IEnumerable<MemberDeclarationSyntax> MakeSkeletonMethods(TypeDefinition def)
        {
            ...
            {
                var methodDecl = MethodDeclaration(
                    _names.Type<Task<Zap.Rpc.AnswerOrCounterquestion>>(Nullability.NonNullable),
                    _names.GetCodeIdentifier(method).Identifier)
        ...
        public MemberDeclarationSyntax MakeSkeleton(TypeDefinition type)
        {
            ...
            var classDecl = ClassDeclaration(name)
                ...
                .AddBaseListTypes(
                    SimpleBaseType(
                        GenericName(nameof(Zap.Rpc.Skeleton))
                            .AddTypeArgumentListArguments(
                ...
                .AddMembers(
                    ...
                    // InterfaceId
                    PropertyDeclaration(_names.Type<ulong>(Nullability.NonNullable), nameof(Zap.Rpc.Skeleton<object>.InterfaceId))
            ...
            foreach (var method in type.Methods)
            {
                foreach (var path in ExpandPipeliningPaths(method))
                {
                    ...
                    var pathDecl = FieldDeclaration(
                        VariableDeclaration(
                            IdentifierName(nameof(Zap.Rpc.MemberAccessPath)))
                        .AddVariables(
                            VariableDeclarator(
                            ...
                            ReturnStatement(
                                CastExpression(
                                    ...
                                    InvocationExpression(
                                        MemberAccessExpression(
                                            SyntaxKind.SimpleMemberAccessExpression,
                                            IdentifierName(nameof(Zap.Rpc.CapabilityReflection)),
                                            GenericName(
                                                Identifier(nameof(Zap.Rpc.CapabilityReflection.CreateProxy)))
                                            .AddTypeArgumentListArguments(
                                                capTypeSyntax)))
                                    .AddArgumentListArguments(
                                        Argument(
                                            InvocationExpression(
                                                MemberAccessExpression(
                                                    SyntaxKind.SimpleMemberAccessExpression,
                                                    IdentifierName(nameof(Zap.Rpc.Impatient)),
                                                    IdentifierName(nameof(Zap.Rpc.Impatient.Access))))
                                            .AddArgumentListArguments(
                                                Argument(
                                                    _names.TaskParameter.IdentifierName),
                                                Argument(
