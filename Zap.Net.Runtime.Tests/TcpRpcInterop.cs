...
using Zap.Net.Runtime.Tests.Util;
using Zap.Rpc;
...
namespace Zap.Net.Runtime.Tests
{
    ...
    public class TcpRpcInterop: TestBase
    {
        ...
        Process StartProcess(ProcessStartInfo processStartInfo)
        {
            ...
            {
                Assert.Fail($"Did not find test executable {processStartInfo.FileName}. Did you build ZapCompatTest.sln in Release configuration?");
            }
        ...
        bool TryLaunchCompatTestProcess(IPAddress addr, int port, string whichTest, Action<StreamReader> test)
        {
            ...
            config = "Release";
#endif
            string path = Path.Combine(myPath, $@"..\..\..\..\{config}\ZapCompatTest.exe");
        ...
        [TestMethod, Ignore]
        // For details on why this test is ignored, see https://github.com/zap-proto/cs/issues/876
            ...
            LaunchCompatTestProcess("server:MoreStuff", addr, port, stdout =>
            {
                ...
                using (var client = new TcpRpcClient(addr.ToString(), port))
                {
                    ...
                    using (var main = client.GetMain<ITestMoreStuff>())
                    {
                        ...
                        // At this point, we have a major difference to ZAP original test suite.
