...
namespace ZapGen
{
    ...
    public class GenericAProxy<TT> : Proxy, IGenericA<TT> where TT : class
    {
        ...
        {
            var in_ = SerializerState.CreateForRpc<ZapGen.GenericA<TT>.Params_methodA.WRITER>();
            var arg_ = new ZapGen.GenericA<TT>.Params_methodA()
            {Param1 = param1};
            arg_.serialize(in_);
            var d_ = await Call(10341943558714247093UL, 0, in_.Rewrap<DynamicSerializerState>(), false, cancellationToken_).WhenReturned;
            var r_ = ZapSerializable.Create<ZapGen.GenericA<TT>.Result_methodA>(d_);
            return;
        }
    }
    ...
    public class GenericASkeleton<TT> : Skeleton<IGenericA<TT>> where TT : class
    {
        ...
        {
            var in_ = ZapSerializable.Create<ZapGen.GenericA<TT>.Params_methodA>(d_);
            await Impl.MethodA(in_.Param1, cancellationToken_);
            var s_ = SerializerState.CreateForRpc<ZapGen.GenericA<TT>.Result_methodA.WRITER>();
            return s_;
        }
    }
        ...
        where TT : class
    {
        ...
        public class Params_methodA : IZapSerializable
        {
            ...
            void IZapSerializable.Deserialize(DeserializerState arg_)
            {
                ...
                Param1 = ZapSerializable.Create<TT>(reader.Param1);
                applyDefaults();
            }
    ...
    public interface IB2 : ZapGen.IGenericA<string>
    {
    ...
    public class B2Proxy : Proxy, IB2
    {
        ...
        {
            var in_ = SerializerState.CreateForRpc<ZapGen.B2.Params_methodB.WRITER>();
            var arg_ = new ZapGen.B2.Params_methodB()
            {Param1 = param1};
            arg_.serialize(in_);
            var d_ = await Call(16414937344734980809UL, 0, in_.Rewrap<DynamicSerializerState>(), false, cancellationToken_).WhenReturned;
            var r_ = ZapSerializable.Create<ZapGen.B2.Result_methodB>(d_);
            return (r_.Res);
        }

        public async Task MethodA(string param1, CancellationToken cancellationToken_ = default)
        {
            var in_ = SerializerState.CreateForRpc<ZapGen.GenericA<string>.Params_methodA.WRITER>();
            var arg_ = new ZapGen.GenericA<string>.Params_methodA()
            {Param1 = param1};
            arg_.serialize(in_);
            var d_ = await Call(10341943558714247093UL, 0, in_.Rewrap<DynamicSerializerState>(), false, cancellationToken_).WhenReturned;
            var r_ = ZapSerializable.Create<ZapGen.GenericA<string>.Result_methodA>(d_);
            return;
        }
    }
    ...
    public class B2Skeleton : Skeleton<IB2>
    {
        ...
        {
            var in_ = ZapSerializable.Create<ZapGen.B2.Params_methodB>(d_);
            return Impatient.MaybeTailCall(Impl.MethodB(in_.Param1, cancellationToken_), res =>
            {
                var s_ = SerializerState.CreateForRpc<ZapGen.B2.Result_methodB.WRITER>();
                var r_ = new ZapGen.B2.Result_methodB{Res = res};
                r_.serialize(s_);
                return s_;
            }
    ...
    public static class B2
    {
        ...
        public class Params_methodB : IZapSerializable
        {
        ...
        public class Result_methodB : IZapSerializable
        {
